...
ledger.Open(ctx,
	...
	ledger.WithSyncer(ledger.NewFsyncSyncer()),  // Fsync/Fdatasync/Full/Noop, default Fsync → Full on Darwin
...
- `NewFsyncSyncer()` - `fsync` on Linux, `fsync` on Darwin
- `NewFdatasyncSyncer()` - `fdatasync` on Linux (data only, cheaper for sequential), falls back to `fsync` on Darwin
- `NewFullSyncer()` / `NewFullFSyncSyncer()` - `F_FULLFSYNC` via `fcntl 51` on Darwin (flush drive cache), `fsync` on Linux

Only `darwin` and `linux` are supported for the real syncers.

## Durability

On a correctly configured host (drive honors flush, filesystem with barriers), `Append` returns only after `Sync` i.e `fsync` on Linux, `F_FULLFSYNC` on Darwin. So the prefix of complete records survives process, OS and power failure.
